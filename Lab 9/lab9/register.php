...
?>
	...
	<div class="container">
		...
		<form action="<?=$_SERVER['PHP_SELF']?>" method="POST">
			...
			<br><input class="btn btn-info" type="submit" name="submit" value="Submit"><br>
		</form>
...
<?php
		...
		if($username == ''){
			...
			echo "Sorry, username cannot be more than 20 characters.";
		}
                else if($password == ''){
                	...
                	$insert_query = "INSERT INTO user (username,salt,hashed_password,user_type) VALUES (?,?,?,?)";
			...
			$pwhash = sha1($salt . $password);
                	...
                	if($stmt = mysqli_prepare($link, $insert_query)){
                        	mysqli_stmt_bind_param($stmt,"ssss",htmlspecialchars($username),htmlspecialchars($salt),htmlspecialchars($pwhash), htmlspecialchars($usertype));
                        	mysqli_stmt_execute($stmt);
                        	mysqli_stmt_close($stmt);
